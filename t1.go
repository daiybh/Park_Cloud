package main

import (
	"fmt"

	jsoniter "github.com/json-iterator/go"
)

//{"service_name":"login","sign":"15E3DF039F5A02BB1A17316976DE8A51","data":{"union_id":"200128","park_id":"24155","local_id":"c61562ee71b6_2.1.0.0_channels_1_2_3_4_5_6_7_8_9_10_11_12_13_14_15_16","version":"2.1.0.0"}}
func parseJSON(jsonStr []byte) (ret string, err error) {
	fmt.Println(string(jsonStr))
	ccc := jsoniter.Get(jsonStr, "service_name").ToString()

	fmt.Println(ccc)
	if ccc == "login" {
		return `{"state":1,"token":"5880277f494544259642dd7ac35afdf4","service_name":"login"}`, nil
	}
	return `Error`, nil
}

func TwoSum(nums []int, target int) []int {
	fmt.Println("nums", nums)
	m := make(map[int]int)
	for i, num := range nums {
		key := target - num
		if j, ok := m[key]; ok {
			return []int{j, i}
		}
		m[nums[i]] = i
	}
	return []int{}
}
